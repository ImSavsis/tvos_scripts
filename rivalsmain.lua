    ...
    for _, plr in pairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if hum and hum.Health > 0 then
                local part = plr.Character:FindFirstChild(Config.Aimbot.LockPart) or plr.Character.HumanoidRootPart
...
Main.Size = UDim2.new(0, 240, 0, 520); Main.Position = UDim2.new(0.05, 0, 0.2, 0); Main.BackgroundColor3 = Color3.fromRGB(15, 10, 20); Main.BorderSizePixel = 2; Main.BorderColor3 = Config.Visuals.Color; Main.Active = true; Main.Draggable = true

local Title = Instance.new("TextLabel", Main)
Title.Size = UDim2.new(1, 0, 0, 35); Title.Text = "TVOS SCRIPTS V18"; Title.BackgroundColor3 = Color3.fromRGB(30, 0, 60); Title.TextColor3 = Color3.new(1,1,1); Title.Font = Enum.Font.GothamBold

local Content = Instance.new("ScrollingFrame", Main)
Content.Position = UDim2.new(0,0,0,40); Content.Size = UDim2.new(1,0,1,-40); Content.BackgroundTransparency = 1; Content.CanvasSize = UDim2.new(0,0,3.5,0); Content.ScrollBarThickness = 4
Instance.new("UIListLayout", Content).HorizontalAlignment = "Center"; Content.UIListLayout.Padding = UDim.new(0, 5)

-- Функция управления Anti-Aim
local function ManageAA(state)
    local character = LocalPlayer.Character
    local hrp = character and character:FindFirstChild("HumanoidRootPart")
    if not hrp then return end

    local existingAV = hrp:FindFirstChild("TVOS_AA")
    if state then
        if not existingAV then
            ...
            av.AngularVelocity = Vector3.new(0, Config.AntiAim.Speed, 0)
        end
    else
        if existingAV then existingAV:Destroy() end
    end
end
...
local function AddToggle(text, configTable, configValue)
    ...
    btn.MouseButton1Click:Connect(function() 
        ...
        -- Специальная логика для некоторых кнопок
        ...
        elseif text == "Anti-Aim" then
            ManageAA(configTable[configValue])
        end
    end)
end
...
AddToggle("Anti-Aim", Config.AntiAim, "Enabled")
...
local mainLoop = RunService.RenderStepped:Connect(function()
    ...
    if AimRunning and Config.Aimbot.Enabled then
        ...
        if target and target.Character and target.Character:FindFirstChild(Config.Aimbot.LockPart) then
            Camera.CFrame = CFrame.new(Camera.CFrame.Position, target.Character[Config.Aimbot.LockPart].Position)
        end
    end
    ...
    -- Anti-Aim Check (на случай респавна)
...
table.insert(Connections, UserInputService.InputBegan:Connect(function(i, g)
    if not g and (i.KeyCode == Enum.KeyCode.Home or i.KeyCode == Enum.KeyCode.Insert) then 
        Main.Visible = not Main.Visible 
    end
    if i.UserInputType == Config.Aimbot.Key then AimRunning = true end
    
    -- TP Behind
    if not g and i.KeyCode == Config.Movement.TPKey then
        local target = GetClosestPlayerToMouse()
        if target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            local targetHRP = target.Character.HumanoidRootPart
            LocalPlayer.Character.HumanoidRootPart.CFrame = targetHRP.CFrame * CFrame.new(0, 0, -Config.Movement.BehindOffset)
        end
    end
end))
